...
import pandas as pd  # 자연어 처리
import numpy as np  # 기계어 처리
from sklearn import preprocessing
import folium
from dreft.common.models import Reader, Printer, FileDTO
...
예산금액을 입력하면, 구당 할당되는 cctv카운터를 자동으로 
알려주는 AI 프로그램을 작성하시오.
'''
...
class Service(Reader):
    ...
    def __init__(self):  # 모듈이 아닌셋업 의미있는 데이터를 리턴하지 않기때문
        self.f = FileDTO()
        self.r = Reader()
        self.p = Printer()
    ...
    def save_police_pos(self):  # 모듈 의미있는 데이터를 반환하거나 변화를 준다
        f = self.f
        r = self.r
        p = self.p
        f.context = './data/'
    ...
    def save_cctv_pop(self):
        ...
        cctv.drop(['2013년도 이전', '2014년', '2015년', '2016년'], 1, inplace=True)  # 세로 0은 가로
        cctv_pop = pd.merge(cctv, pop, on='구별')  # 엑셀 파일 결합
    ...
    def save_folium_map(self):
        # polnorm.csv kr sta.json, crimein.csv policepos
        f = self.f
        r = self.r
        p = self.p
        # csv를 df로 객채로 정의한다(객체전환)
        f.context = './saved_data/'
        f.fname = 'police_norm'
        police_norm = r.csv(f)
        ...
        folium_map = folium.Map(location=[37.5502, 126.982], zoom_start=12, tiles='Stamen Toner')

        state_geo = "./data/Kr-states.json"  # 한국지도
        ...
        folium.Choropleth(
            ...
            # df구조는 변할수있는상태 (variable)이기때문에 튜플에 담아 const 상태로 만들어 지도에 그린다
            # 튜플로 컨버전하는 메소드 ()-> 튜플이긴하지만 저기있는 튜플은 메소드
        ...
        folium_map.save('./saved_data/seoul_crime.html')  # 장고는 html이나 json으로 던지기 때문에 html로하고 리엑트에 던져준다
